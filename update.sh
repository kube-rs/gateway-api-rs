...
do
    ...
    curl -sSL "https://raw.githubusercontent.com/kubernetes-sigs/gateway-api/main/config/crd/standard/gateway.networking.k8s.io_${API}.yaml?ref=${VERSION}" | kopium --schema=derived --derive=JsonSchema --derive=Default --docs -f - > src/apis/standard/${API}.rs
...
do
    ...
    curl -sSL "https://raw.githubusercontent.com/kubernetes-sigs/gateway-api/main/config/crd/experimental/gateway.networking.k8s.io_${API}.yaml?ref=${VERSION}" | kopium --schema=derived --derive=JsonSchema --derive=Default --docs -f - > src/apis/experimental/${API}.rs
